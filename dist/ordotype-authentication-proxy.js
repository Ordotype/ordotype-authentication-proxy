  ...
  if (console.log("getApp"), !!m())
    try {
      if (await d.validateSessionStatus() === !1) {
        await window.$memberstackDom.logout(), window.location.href = "/";
        return;
      }
    } catch (o) {
      if (o instanceof c) {
        (o.status === 401 || o.status === 403) && await window.$memberstackDom.logout(), window.location.href = "/";
        return;
      }
    }
});
document.addEventListener(i.LOGOUT, async () => {
  console.log("logout"), await d.logout(), localStorage.removeItem("_ms-mid"), localStorage.removeItem("_ms_mem");
});
